...
int sHist[768]; 												/* R+G+B histogram “buckets” */

/* Lock for each histogram */
pthread_mutex_t rHistLock, gHistLock, bHistLock, sHistLock = PTHREAD_MUTEX_INITIALIZER;

/* Lock creation for each histogram */
pthread_mutex_t rHistGroupLocks[8], gHistGroupLocks[8], bHistGroupLocks[8];
pthread_mutex_t sHistGroupLocks[24];

/* Lock creation for each index in histograms */
pthread_mutex_t rHistIndexLocks[256], gHistIndexLocks[256], bHistIndexLocks[256];
pthread_mutex_t sHistIndexLocks[768];


/* Populates histograms where each histogram has a lock corresponding to it
 *
 * params
 * r The rank of the thread
 */
void* lock_each(void* r) {

	frame_ptr from = args->from;

	// Each thread gets 1/N of the rows
	int chunks = from->image_height / num_procs;
	long rank = (long)r;
	...
	for (i=index_from; i < index_to; i++)
		for (j=0; j < from->image_width; j++) {
				red_pixel = from->row_pointers[i][(from->num_components)*j+0];  // k = 0 for red
				green_pixel = from->row_pointers[i][(from->num_components)*j+1];  // k = 1 for green
				blue_pixel = from->row_pointers[i][(from->num_components)*j+2];  // k = 2 for blue

				pthread_mutex_lock(&rHistLock);
				rHist[red_pixel]++;
				pthread_mutex_unlock(&rHistLock);

				pthread_mutex_lock(&gHistLock);
				gHist[green_pixel]++;
				pthread_mutex_unlock(&gHistLock);

				pthread_mutex_lock(&bHistLock);
				bHist[blue_pixel]++;
				pthread_mutex_unlock(&bHistLock);

				pthread_mutex_lock(&sHistLock);
				sHist[red_pixel + green_pixel + blue_pixel]++;
				pthread_mutex_unlock(&sHistLock);
		}

	return NULL;
}

/* Populates histograms where there is a lock for every 32 indices of the
 * histograms
 *
 * params
 * r The rank of the thread
 */
void* lock_group(void* r) {
	...
	for (i=index_from; i < index_to; i++)
		for (j=0; j < from->image_width; j++) {
				...
				int red_index = red_pixel/32;
				int green_index = green_pixel/32;
				int blue_index = blue_pixel/32;
				int sum_index = (red_pixel + green_pixel + blue_pixel)/32;

				pthread_mutex_lock(&rHistGroupLocks[red_index]);
				rHist[red_pixel]++;
				pthread_mutex_unlock(&rHistGroupLocks[red_index]);

				pthread_mutex_lock(&gHistGroupLocks[green_index]);
				gHist[green_pixel]++;
				pthread_mutex_unlock(&gHistGroupLocks[green_index]);

				pthread_mutex_lock(&bHistGroupLocks[blue_index]);
				bHist[blue_pixel]++;
				pthread_mutex_unlock(&bHistGroupLocks[blue_index]);

				pthread_mutex_lock(&sHistGroupLocks[sum_index]);
				sHist[red_pixel + green_pixel + blue_pixel]++;
				pthread_mutex_unlock(&sHistGroupLocks[sum_index]);
		}

	return NULL;
}

/* Populates histograms where each index has a lock corresponding to it
 *
 * params
 * r The rank of the thread
 */
void* lock_index(void* r) {
	...
	for (i=index_from; i < index_to; i++)
		for (j=0; j < from->image_width; j++) {
				...
				pthread_mutex_lock(&rHistIndexLocks[red_pixel]);
				rHist[red_pixel]++;
				pthread_mutex_unlock(&rHistIndexLocks[red_pixel]);

				pthread_mutex_lock(&gHistIndexLocks[green_pixel]);
				gHist[green_pixel]++;
				pthread_mutex_unlock(&gHistIndexLocks[green_pixel]);

				pthread_mutex_lock(&bHistIndexLocks[blue_pixel]);
				bHist[blue_pixel]++;
				pthread_mutex_unlock(&bHistIndexLocks[blue_pixel]);

				int sum_index = red_pixel + green_pixel + blue_pixel;
				pthread_mutex_lock(&sHistIndexLocks[sum_index]);
				sHist[sum_index]++;
				pthread_mutex_unlock(&sHistIndexLocks[sum_index]);
		}
...
void setUpLocks() {
	#ifdef ENTIRE_LOCK
	pthread_mutex_init(&rHistLock, NULL); pthread_mutex_init(&gHistLock, NULL);
	pthread_mutex_init(&bHistLock, NULL); pthread_mutex_init(&sHistLock, NULL);
	#endif

	#ifdef GROUP_LOCK

	for (int i = 0; i < 8; i++) {
		pthread_mutex_init(&rHistGroupLocks[i], NULL);
		pthread_mutex_init(&gHistGroupLocks[i], NULL);
		pthread_mutex_init(&bHistGroupLocks[i], NULL);
	}

	for (int i = 0; i < 24; i++) {
		pthread_mutex_init(&sHistGroupLocks[i], NULL);
	}
	#endif

	#ifdef INDEX_LOCK
	for (int i = 0; i < 256; i++) {
		pthread_mutex_init(&rHistIndexLocks[i], NULL);
		pthread_mutex_init(&gHistIndexLocks[i], NULL);
		pthread_mutex_init(&bHistIndexLocks[i], NULL);
	}

	for (int i = 0; i < 768; i++) {
		pthread_mutex_init(&sHistIndexLocks[i], NULL);
	}
	#endif
}
...
void CS338_function()
{
	...
	// Initialize every array to 0s
	memset(rHist, 0, sizeof(int) * 256); memset(gHist, 0, sizeof(int) * 256);
	memset(bHist, 0, sizeof(int) * 256); memset(sHist, 0, sizeof(int) * 768);
	...
	for(long i = 0; i < num_procs; i++){
		pthread_create(&ids[i], NULL, lock_each, (void*)i);
	}
	#endif

	#ifdef GROUP_LOCK
	for(long i = 0; i < num_procs; i++){
		pthread_create(&ids[i], NULL, lock_group, (void*)i);
	}
	#endif

	#ifdef INDEX_LOCK
	for(long i = 0; i < num_procs; i++){
		pthread_create(&ids[i], NULL, lock_index, (void*)i);
	}
	#endif
	...
	for (int i=0; i < 256; i++)
		fprintf(file, "%3u: R:%8u G:%8u B:%8u S0:%8u S1:%8u S2:%8u\n",
