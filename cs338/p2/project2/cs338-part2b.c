...
void* private_histogram(void* r) {
	...
	struct Histograms* result	= malloc (sizeof (struct Histograms));
	memcpy(&result -> rHist, &rHist, sizeof(rHist));
	memcpy(&result -> gHist, &gHist, sizeof(gHist));
	memcpy(&result -> bHist, &bHist, sizeof(bHist));
	memcpy(&result -> sHist, &sHist, sizeof(sHist));
...
void CS338_function()
{
	...
	for(long i = 0; i < num_procs; i++){
		pthread_create(&ids[i], NULL, private_histogram, (void*)i);
	}
	...
	for(int i = 0; i < num_procs; i++){
		void *returnValue;
		pthread_join(ids[i], &returnValue);
		struct Histograms* returnedHistograms = (struct Histograms*)returnValue;
		for (int i = 0; i < 256; i++) {
			rHist[i] += returnedHistograms->rHist[i];
			bHist[i] += returnedHistograms->bHist[i];
			gHist[i] += returnedHistograms->gHist[i];
			sHist[i] += returnedHistograms->sHist[i];
			sHist[i+256] += returnedHistograms->sHist[i+256];
			sHist[i+512] += returnedHistograms->sHist[i+512];
		}
	}

	FILE* file = fopen("output_private.txt", "w");
	if (file == NULL) {
